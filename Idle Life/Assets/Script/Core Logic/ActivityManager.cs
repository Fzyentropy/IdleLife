...
public class ActivityManager : MonoBehaviour
{
    ...
    private Activity Current_Activity;  // 当前 Activity 容器

    public Activity current_activity    // 当前活动属性 连接和更改容器存储的 Activity
    {
    ...
    {
        var act = current_activity;
        var tick_interval = new WaitForSeconds(act.Activity_Duration);   // 设置此协程的单次 Tick 时长
        
                // TODO 也许要扩展一下进度条的显示
        
        while (act == current_activity && act.Can_Start_Activity())   // Tick 循环 Loop
        {
            yield return tick_interval;
            
            // act.OnTick?.Invoke();
            act.Activity_Outcome_Tick();    // 一次 Tick 执行
    ...
    private void LoadAllActivities()
    {
        ...
        // 加载所有Activity配置
        var activityDataArray = Resources.LoadAll<Activity_Data_Scriptable>(PATH_SCRIPTABLE_OBJECTS_ACTIVITY);
        
        foreach (var activity_instance in activityDataArray)
        {
            var activity = CreateActivityFromData(activity_instance);
            if (activity != null)
            {
    ...
    private Activity CreateActivityFromData(Activity_Data_Scriptable activityInstance)
    {
        if (activityInstance.Activity_Type == "Study")
        {
            try
            {
